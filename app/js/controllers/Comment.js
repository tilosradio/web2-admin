...
angular.module('tilosAdmin').controller('CommentsCtrl', function ($http, API_SERVER_ENDPOINT, $scope) {
    ...
    $scope.approve = function (id) {
        $http.post(API_SERVER_ENDPOINT + '/api/v1/comment/approve/' + id, {}).success(function (data) {
            $http.get(API_SERVER_ENDPOINT + '/api/v1/comment?status=' + status).success(function (data) {
                $scope.comments = data;
            });
        });
    ...
    $scope.delete = function (id) {
        $http.delete(API_SERVER_ENDPOINT + '/api/v1/comment/approve/' + id, {}).success(function (data) {
            $http.get(API_SERVER_ENDPOINT + '/api/v1/comment?status=' + status).success(function (data) {
                $scope.comments = data;
            });
        });
